...
class RequestBuilder<T: Decodable> {
    ...
    private func excute(_ completion: @escaping (_ response: Response<CommonDTO<T>>?, _ error: Error?) -> Void) {
        makeRequest().responseDecodable(of: CommonDTO<T>.self) { result in
        ...
        return Observable.create { observer -> Disposable in
            self.excute { response, error in
                ...
                if let data = response?.data {
                    observer.onNext(data.responseObject)
